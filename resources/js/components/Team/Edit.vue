    ...
    <div class="container mx-auto max-w-3xl">
        <form @submit.prevent="update" @keydown="form.onKeydown($event)" enctype="multipart/form-data">
                ...
                <div class="py-8 px-16 clearfix">
                    <label for="profile_picture" class="block uppercase tracking-wide text-gray-600 text-xs font-bold mb-2">{{ $t('profile_picture') }}</label>
                     <img class="rounded-full w-16 h-16 border-4 mt-2 border-gray-200 float-left" 
                          :src="form.profile_pic ? getProfilePhoto() : 'https://www.gravatar.com/avatar/faf1084144ec566e51fadd3420c66c02?s=100&amp;d=mm&amp;r=g'" 
                          alt="photo">
            ...
            <div class="p-16 py-8 bg-gray-300 clearfix rounded-b-lg border-t border-gray-200">           
                <router-link :to="{ name: 'settings.team'  }" class="text-sm rounded-lg shadow inline-flex items-center bg-pink-500 focus:outline-none focus:shadow-outline text-white py-2 px-4">
...
export default {
    ...
    methods: {
        ...
        async getData(){
            ...
            this.form.role = data.data.roles[0];
        },
        ...
        getProfilePhoto(){
             let prefix = (this.form.profile_pic.match(/\//) ? '' : window.location.origin+'/img/profile/');
             return prefix + this.form.profile_pic;
        },
        ...
        async update() {
            ...
            await this.form.patch(window.location.origin+'/api/team/'+this.$route.params.slug)
                .then((response)=>{
                    toast.fire({
                        type: 'success',
