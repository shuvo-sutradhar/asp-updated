<template>
  <div class="flex flex-col items-center h-screen bg-gray-200 p-4 rotate">
      ...
      <div class="sm:max-w-xl md:max-w-3xl w-full m-auto">
           ...
           <form @submit.prevent="login" @keydown="form.onKeydown($event)" class="flex items-stretch bg-white rounded-lg shadow-lg overflow-hidden border-t-4 border-indigo-500 sm:border-0">
              <!-- @csrf -->
              <div class="flex hidden overflow-hidden relative sm:block w-5/12 md:w-6/12 bg-gray-600 text-gray-300 py-4 bg-cover bg-center" style="background-image: url('https://images.unsplash.com/photo-1477346611705-65d1883cee1e?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=1350&q=80')">
                  <div class="flex-1 absolute bottom-0 text-white p-10">
                      <h3 class="text-4xl font-bold inline-block">{{ $t('login') }}</h3>
              ...
              <div class="flex-1 p-6 sm:p-8 sm:py-12">
                  ...
                  {{ $t('login') }} <span class="text-gray-400 font-light">{{ $t('to_your_account') }}</span></h3>
                  <!-- Email -->
                  <input id="email" 
                         type="email" 
                         v-model="form.email"
                  ...
                  <input v-model="form.password" 
                        ...
                        required>
                  <has-error :form="form" field="password" class="text-red-500 mb-3" />


                  <checkbox v-model="remember" name="remember" class="flex items-center mb-3">
                    {{ $t('remember_me') }}
                  </checkbox>
                  ...
                  <p class="w-full text-sm text-left text-gray-700 mt-8">
                      {{ $t('dont_have_account') }}
                      <router-link :to="{ name: 'register' }" class="text-blue-500 hover:text-blue-700 no-underline font-medium">
                          {{ $t('register') }}
                      </router-link>
                  </p>
